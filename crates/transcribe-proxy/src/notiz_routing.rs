...
#[derive(Debug, Clone)]
pub struct NotizRoutingConfig {
    pub priorities: Vec<Provider>,
    pub retry_config: RetryConfig,
}

impl Default for NotizRoutingConfig {
    fn default() -> Self {
        Self {
            priorities: vec![
                Provider::Deepgram,
...
impl NotizRouter {
    pub fn new(config: NotizRoutingConfig) -> Self {
        Self {
...
impl Default for NotizRouter {
    fn default() -> Self {
        Self::new(NotizRoutingConfig::default())
    }
}
...
pub fn should_use_notiz_routing(provider_param: Option<&str>) -> bool {
    provider_param == Some("notiz")
}
...
mod tests {
    ...
    fn select_provider_none_when_no_available() {
        let router = NotizRouter::default();
        let selected = router.select_provider(&langs(&[ISO639::En]), &HashSet::new());
        assert_eq!(selected, None);
    }

    #[test]
    fn should_use_notiz_routing() {
        assert!(super::should_use_notiz_routing(Some("notiz")));

        assert!(!super::should_use_notiz_routing(None));
        assert!(!super::should_use_notiz_routing(Some("deepgram")));
        assert!(!super::should_use_notiz_routing(Some("soniox")));
        assert!(!super::should_use_notiz_routing(Some("assemblyai")));
        assert!(!super::should_use_notiz_routing(Some("")));
        assert!(!super::should_use_notiz_routing(Some("auto")));
    }
    ...
    fn default_priorities_include_newer_live_providers() {
        let config = NotizRoutingConfig::default();

...
use skald_audio_mime::content_type_to_extension;
...
pub async fn handler(
    ...
    let use_skald_routing = should_use_skald_routing(provider_param.as_deref());

    if use_skald_routing {
        return sync::handle_skald_batch(
            &state,
            &params,
            listen_params,
            audio.path(),
    ...
    tracing::info!(
        ...
        skald.file.mime_type = %content_type,
        skald.payload.size_bytes = %audio.len(),

...
use notiz_audio_mime::content_type_to_extension;
...
pub async fn handler(
    ...
    let use_notiz_routing = should_use_notiz_routing(provider_param.as_deref());

    if use_notiz_routing {
        return sync::handle_notiz_batch(
            &state,
            &params,
            listen_params,
            audio.path(),
    ...
    tracing::info!(
        ...
        notiz.file.mime_type = %content_type,
        notiz.payload.size_bytes = %audio.len(),

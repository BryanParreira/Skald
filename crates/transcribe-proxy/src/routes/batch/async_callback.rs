...
> {
    tracing::info!(
        notiz.stt.provider.name = %provider_str,
        "local_url_detected, using sync transcription"
    );

    let download_response =
        notiz_observability::with_current_trace_context(state.client.get(audio_url))
            .send()
            .await
    ...
    if !download_status.is_success() || audio_bytes.len() < 1024 {
        ...
        tracing::error!(
            ...
            notiz.audio.size_bytes = audio_bytes.len(),
            notiz.file.id = %file_id,

...
> {
    tracing::info!(
        skald.stt.provider.name = %provider_str,
        "local_url_detected, using sync transcription"
    );

    let download_response =
        skald_observability::with_current_trace_context(state.client.get(audio_url))
            .send()
            .await
    ...
    if !download_status.is_success() || audio_bytes.len() < 1024 {
        ...
        tracing::error!(
            ...
            skald.audio.size_bytes = audio_bytes.len(),
            skald.file.id = %file_id,

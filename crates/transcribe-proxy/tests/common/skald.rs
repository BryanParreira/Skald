...
use owhisper_client::{BatchClient, BatchSttAdapter, DeepgramAdapter, ListenClient, SkaldAdapter};
...
fn is_mock_trailing_disconnect(error: &skald_ws_client::Error) -> bool {
...
pub fn english() -> Vec<skald_language::Language> {
    vec![skald_language::ISO639::En.into()]
}
...
pub async fn send_streaming(addr: SocketAddr, query: &str) {
    let url = format!(
        "ws://{addr}/listen?provider=skald&encoding=linear16&sample_rate=16000&channels=1&{query}"
    );
...
pub async fn send_streaming_via_client(
    ...
    let client = skald_listen_client(addr, model, languages).await;
...
pub async fn collect_streaming_via_client_result(
    ...
    let (stream, _handle) = client
        ...
        .await
        .expect("skald streaming client should connect");
...
pub async fn send_batch(addr: SocketAddr, query: &str) {
    let resp = reqwest::Client::new()
        .post(format!("http://{addr}/listen?provider=skald&{query}"))
...
pub async fn send_batch_via_skald_client(
    addr: SocketAddr,
    model: &str,
    languages: Vec<skald_language::Language>,
) -> owhisper_interface::batch::Response {
    batch_client::<SkaldAdapter>(addr, model, languages)
        .transcribe_file(skald_data::english_1::AUDIO_PATH)
        .await
        .expect("skald batch request should succeed")
}
...
pub fn stereo_listen_url(addr: SocketAddr, query: &str) -> String {
    format!("ws://{addr}/listen?provider=skald&sample_rate=16000&channels=2&{query}")
}

async fn skald_listen_client(
    addr: SocketAddr,
    model: &str,
    languages: Vec<skald_language::Language>,
) -> ListenClient<SkaldAdapter> {
    ListenClient::builder()
        .adapter::<SkaldAdapter>()

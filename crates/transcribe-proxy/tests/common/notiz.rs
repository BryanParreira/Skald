...
use owhisper_client::{BatchClient, BatchSttAdapter, DeepgramAdapter, ListenClient, NotizAdapter};
...
fn is_mock_trailing_disconnect(error: &notiz_ws_client::Error) -> bool {
...
pub fn english() -> Vec<notiz_language::Language> {
    vec![notiz_language::ISO639::En.into()]
}
...
pub async fn send_streaming(addr: SocketAddr, query: &str) {
    let url = format!(
        "ws://{addr}/listen?provider=notiz&encoding=linear16&sample_rate=16000&channels=1&{query}"
    );
...
pub async fn send_streaming_via_client(
    ...
    let client = notiz_listen_client(addr, model, languages).await;
...
pub async fn collect_streaming_via_client_result(
    ...
    let (stream, _handle) = client
        ...
        .await
        .expect("notiz streaming client should connect");
...
pub async fn send_batch(addr: SocketAddr, query: &str) {
    let resp = reqwest::Client::new()
        .post(format!("http://{addr}/listen?provider=notiz&{query}"))
...
pub async fn send_batch_via_notiz_client(
    addr: SocketAddr,
    model: &str,
    languages: Vec<notiz_language::Language>,
) -> owhisper_interface::batch::Response {
    batch_client::<NotizAdapter>(addr, model, languages)
        .transcribe_file(notiz_data::english_1::AUDIO_PATH)
        .await
        .expect("notiz batch request should succeed")
}
...
pub fn stereo_listen_url(addr: SocketAddr, query: &str) -> String {
    format!("ws://{addr}/listen?provider=notiz&sample_rate=16000&channels=2&{query}")
}

async fn notiz_listen_client(
    addr: SocketAddr,
    model: &str,
    languages: Vec<notiz_language::Language>,
) -> ListenClient<NotizAdapter> {
    ListenClient::builder()
        .adapter::<NotizAdapter>()

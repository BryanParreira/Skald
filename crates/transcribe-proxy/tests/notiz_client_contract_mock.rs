...
use common::{
    ...
    sample_response, send_batch_via_deepgram_client, send_batch_via_notiz_client,
...
#[tokio::test]
async fn streaming_notiz_client_accepts_single_proxy_response_object() {
...
#[tokio::test]
async fn streaming_notiz_client_accepts_single_proxy_error_response_object() {
    let upstream = start_mock_server_with_config(
...
#[tokio::test]
async fn streaming_notiz_client_accepts_single_proxy_rate_limit_error_object() {
    let upstream = start_mock_server_with_config(
        single_response_recording(&sample_error_response(
            4429,
...
#[tokio::test]
async fn streaming_notiz_client_surfaces_soniox_provider_failure() {
    let upstream = start_mock_server_with_config(
...
#[tokio::test]
async fn streaming_notiz_client_surfaces_abnormal_close_without_response() {
    let upstream = start_mock_server_with_config(
...
#[tokio::test]
async fn batch_client_notiz_adapter_uses_proxy_sync_path_under_stt() {
    ...
    let response = send_batch_via_notiz_client(proxy, "cloud", english()).await;
    ...
    assert!(
        ...
        "notiz sync batch should resolve cloud -> nova-3 before upstream: {query}"
    );
    assert!(

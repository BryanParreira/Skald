...
struct BridgeState {
    executor: skald_db_execute::DbExecutor,
    live_query_runtime: Arc<skald_db_reactive::LiveQueryRuntime<ListenerSink>>,
...
impl MobileDbBridge {
    ...
    pub fn open(db_path: String, cloudsync_open_mode: Option<String>) -> Result<Self, BridgeError> {
        ...
        let executor = skald_db_execute::DbExecutor::new(std::sync::Arc::clone(&db));
        let live_query_runtime = {
            let _guard = runtime.enter();
            Arc::new(skald_db_reactive::LiveQueryRuntime::new(db))
        };
    ...
    pub fn execute_proxy(
        ...
        let params = parse_params_json(&params_json)?;
        let method = method
            .parse::<skald_db_execute::ProxyQueryMethod>()
            .map_err(execute_error)?;
        let (runtime, executor) =
            self.with_state(|state| Ok((Arc::clone(&state.runtime), state.executor.clone())))?;
        let rows = runtime
    ...
    pub fn subscribe(
        ...
        if let skald_db_reactive::DependencyAnalysis::NonReactive { reason } =
    ...
    pub fn configure_cloudsync(&self, config_json: String) -> Result<(), BridgeError> {
        let config: skald_db_core::CloudsyncRuntimeConfig = serde_json::from_str(&config_json)
...
mod tests {
    ...
    fn execute_proxy_roundtrips_positional_rows() {
        ...
        let result_json = bridge
            ...
            .unwrap();
        let result: skald_db_execute::ProxyQueryResult =

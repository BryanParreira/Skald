...
use {
    ...
    reqwest::StatusCode,
    skald_download_interface::DownloadProgress,
    std::{
        cmp::min,
        fs::File,
        fs::OpenOptions,
...
mod tests {
    ...
    fn test_calculate_file_size_and_checksum() {
        let base = "/Users/yujonglee/dev/skald/.cache";
    ...
    async fn test_download_file_parallel_s3() {
        ...
        let url = "https://storage2.hyprnote.com/v0/yujonglee/skald-llm-sm/model_q4_k_m.gguf";
        let test_client = reqwest::Client::builder().http1_only().build().unwrap();

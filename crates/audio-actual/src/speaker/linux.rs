        ...
        .param_changed(|_, user_data, id, param| {
            ...
            if user_data.format.parse(param).is_ok() {
                ...
                if rate > 0 {
                    ...
                    tracing::info!(
                        notiz.audio.sample_rate_hz = rate,
...
fn pulseaudio_capture_loop(
    ...
    let mut context = PaContext::new(&mainloop, "notiz-speaker-capture")
    ...
    let setup_result = (|| -> Result<_> {
        ...
        tracing::info!(notiz.audio.device = %monitor_device, "connecting_to_monitor_device");

        mainloop.lock();
        let stream_result = (|| -> Result<_> {
            let mut stream = PaStream::new(&mut context, "notiz-capture", &spec, None)
    ...
    tracing::info!(
        notiz.audio.sample_rate_hz = actual_rate,

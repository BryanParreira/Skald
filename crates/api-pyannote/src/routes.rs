...
use skald_pyannote_cloud::ClientInfo;
...
#[derive(Clone)]
struct AppState {
    client: skald_pyannote_cloud::Client,
}
...
) -> Result<skald_pyannote_cloud::types::DiarizeRequest> {
...
) -> Result<skald_pyannote_cloud::types::IdentifyRequest> {
    body.url = validate_media_url(user_id, &body.url)?;
    Ok(body.into())
}
...
) -> Result<skald_pyannote_cloud::types::VoiceprintRequest> {
    body.url = validate_media_url(user_id, &body.url)?;
    Ok(body.into())
}
...
mod tests {
    ...
    use skald_api_auth::{AuthContext, Claims};

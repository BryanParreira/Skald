...
use notiz_pyannote_cloud::ClientInfo;
...
#[derive(Clone)]
struct AppState {
    client: notiz_pyannote_cloud::Client,
}
...
) -> Result<notiz_pyannote_cloud::types::DiarizeRequest> {
...
) -> Result<notiz_pyannote_cloud::types::IdentifyRequest> {
    body.url = validate_media_url(user_id, &body.url)?;
    Ok(body.into())
}
...
) -> Result<notiz_pyannote_cloud::types::VoiceprintRequest> {
    body.url = validate_media_url(user_id, &body.url)?;
    Ok(body.into())
}
...
mod tests {
    ...
    use notiz_api_auth::{AuthContext, Claims};

...
use notiz_audio::{AudioProvider, CaptureConfig, CaptureFrame, CaptureStream};
use notiz_audio_utils::chunk_size_for_stt;
...
fn handle_capture_item(
    ctx: &StreamContext,
    item: Option<Result<CaptureFrame, notiz_audio::Error>>,
) -> StreamResult {
    match item {
        Some(Ok(frame)) => {

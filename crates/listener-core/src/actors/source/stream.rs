...
use skald_audio::{AudioProvider, CaptureConfig, CaptureFrame, CaptureStream};
use skald_audio_utils::chunk_size_for_stt;
...
fn handle_capture_item(
    ctx: &StreamContext,
    item: Option<Result<CaptureFrame, skald_audio::Error>>,
) -> StreamResult {
    match item {
        Some(Ok(frame)) => {

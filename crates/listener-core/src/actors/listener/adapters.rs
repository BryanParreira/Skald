...
use owhisper_client::{
    ...
    NotizAdapter, RealtimeSttAdapter, SonioxAdapter, notiz_ws_client,
};
...
fn soniqo_model_for_args(
    args: &ListenerArgs,
) -> Result<Option<notiz_transcribe_soniqo::SoniqoModel>, ActorProcessingErr> {
    if let Some(model) =
        notiz_transcribe_soniqo::local_model_from_request(&args.base_url, &args.model)
    {
        return Ok(Some(model));
    }

    if notiz_transcribe_soniqo::is_local_base_url(&args.base_url) {
        return notiz_transcribe_soniqo::SoniqoModel::from_str(&args.model)
            .map(Some)
...
> {
    ...
    let session = tokio::task::spawn_blocking(move || {
        notiz_transcribe_soniqo::LiveTranscriptionSession::start(model)
    })
    .await
                    ...
                    match msg {
                        SoniqoAudioMsg::Single(source, audio) => {
                            match source {
                                ...
                                notiz_transcribe_soniqo::TranscriptSource::System => {
...
async fn flush_soniqo_source(
    session: notiz_transcribe_soniqo::LiveTranscriptionSession,
    model: notiz_transcribe_soniqo::SoniqoModel,
    source: notiz_transcribe_soniqo::TranscriptSource,
    samples: Vec<f32>,
    start: f64,
    duration: f64,
    myself: ActorRef<ListenerMsg>,
    ...
    extra: Extra,
) -> Result<notiz_transcribe_soniqo::LiveTranscriptionSession, String> {
...
fn format_languages(languages: &[notiz_language::Language]) -> String {
...
fn desktop_connect_policy() -> notiz_ws_client::client::WebSocketConnectPolicy {
    notiz_ws_client::client::WebSocketConnectPolicy {
...
> {
    ...
    let client = owhisper_client::ListenClient::builder()
        ...
        .extra_header(DEVICE_FINGERPRINT_HEADER, notiz_host::fingerprint())
        .build_single()
        .await;
    ...
    let (listen_stream, handle) = match client.from_realtime_audio(outbound).await {
        Err(e) => {
            tracing::error!(
                notiz.session.id = %args.session_id,
                error.message = ?e,
...
mod tests {
    ...
    impl notiz_storage::StorageRuntime for NoopRuntime {
        fn global_base(&self) -> Result<std::path::PathBuf, notiz_storage::Error> {
            Ok(std::path::PathBuf::from("/tmp"))
        }

        fn vault_base(&self) -> Result<std::path::PathBuf, notiz_storage::Error> {
            Ok(std::path::PathBuf::from("/tmp"))
        }
    }
    ...
    fn soniqo_model_for_args_accepts_loopback_base_url() {
        ...
        assert_eq!(
            ...
            Some(notiz_transcribe_soniqo::SoniqoModel::ParakeetStreaming)
        );
    }
    ...
    fn format_languages_uses_bcp47_codes() {
        let languages = vec!["en-US".parse().unwrap(), notiz_language::ISO639::Fr.into()];

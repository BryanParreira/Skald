...
meta_description: "Six open-source AI meeting notetakers you can self-host — from full meeting workspaces to raw transcription pipelines. Covers Notiz, Meetily, Scriberr, Vibe, Whishper, and WhisperX."
author:
  - "Harshika"
featured: false
...
| **Notiz**    | MIT         | macOS           | Parakeet V3, Parakeet Batch + 7 cloud | Ollama / LM Studio | Free (local) · $8–$25/mo cloud |
...
![Notiz review](/api/assets/blog/blog/char-summary.png "char-editor-width=80")

[Notiz](/) is the only tool on this list that handles the full meeting workflow in one app — capture, transcription, note-taking, summarization, and structured export.
...
**Install:** brew install notiz, or clone the [MIT-licensed repo](https://github.com/BryanParreira/Notiz) and build from source.

**Transcription:** Pick from 9 STT providers in Preferences. Local Parakeet models run fully on-device, and the other providers are cloud BYOK: paste your API key, audio goes to that provider, Notiz never touches it.
...
- If you want a full meeting workspace, Notiz is the most complete option on this list.
...
For most users who want privacy-first meeting notes that just work, start with [Notiz](/download/apple-silicon). Free for fully local use, no account required.

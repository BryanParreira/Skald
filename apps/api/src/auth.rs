...
pub use notiz_api_auth::{AuthState, optional_auth, require_auth};
...
pub async fn sentry_and_analytics(mut request: Request, next: Next) -> Response {
    ...
    if let Some(auth) = request.extensions().get::<AuthContext>() {
        sentry::configure_scope(|scope| {
            ...
            ctx.insert(
                "notiz.enduser.entitlements".into(),
        ...
        request
            .extensions_mut()
            .insert(notiz_analytics::AuthenticatedUserId(user_id));
    }
    ...
    if let Some(fingerprint) = device_fingerprint {
        ...
        request
            .extensions_mut()
            .insert(notiz_analytics::DeviceFingerprint(fingerprint));
    }

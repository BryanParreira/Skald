...
fn migrate_from_store_json(store_json_path: &Path, auth_path: &Path) -> std::io::Result<()> {
    ...
    notiz_storage::fs::atomic_write(auth_path, &auth_str)?;
    notiz_storage::fs::atomic_write(
        store_json_path,
...
mod test {
    ...
    fn migration_moves_legacy_auth_file() {
        ...
        let legacy_auth_path = temp.path().join("notiz").join(FILENAME);
        let legacy_store_json_path = temp.path().join("notiz").join("store.json");
        let new_auth_path = temp.path().join("com.notiz.stable").join(FILENAME);
    ...
    fn migration_does_not_clone_shared_auth_into_second_bundle() {
        ...
        let legacy_auth_path = temp.path().join("notiz").join(FILENAME);
        let legacy_store_json_path = temp.path().join("notiz").join("store.json");
        let stable_auth_path = temp.path().join("com.notiz.stable").join(FILENAME);
        let nightly_auth_path = temp.path().join("com.notiz.nightly").join(FILENAME);

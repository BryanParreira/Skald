...
pub async fn import_all(source: &ImportSource) -> Result<Collection, crate::Error> {
    match source.transform {
        TransformKind::SkaldV0 => skald::v0::import_all_from_path(&source.path).await,
...
pub async fn import_stats(source: &ImportSource) -> Result<ImportStats, crate::Error> {
    match source.transform {
        TransformKind::SkaldV0 => skald::v0::import_stats_from_path(&source.path).await,
...
pub fn all_sources() -> Vec<ImportSource> {
    [ImportSource::skald_stable(), ImportSource::skald_nightly()]
        .into_iter()
        .flatten()
        .collect()
}

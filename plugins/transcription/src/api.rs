use notiz_transcription_core::{listener, listener2};
...
impl CaptureParams {
    fn default_transcription_mode(&self) -> listener::TranscriptionMode {
        ...
        if let Some(model) =
            notiz_transcribe_soniqo::local_model_from_request(&self.base_url, &self.model)
        {
        ...
        if notiz_transcribe_soniqo::is_local_base_url(&self.base_url) {
            return listener::TranscriptionMode::Batch;
        }
...
mod tests {
    ...
    use notiz_transcription_core::listener::TranscriptionMode;
    ...
    #[test]
    fn defaults_notiz_cloud_en_ko_capture_to_live_mode() {
        let params = capture_params_with_languages(
